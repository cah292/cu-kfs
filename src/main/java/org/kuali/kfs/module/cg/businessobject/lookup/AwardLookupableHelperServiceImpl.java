...
public class AwardLookupableHelperServiceImpl extends KualiLookupableHelperServiceImpl {
    ...
    protected AnchorHtmlData getInvoicesLookupUrl(BusinessObject bo) {
        Properties params = builInvoicesLookupUrlParameters(bo);
        String url = buildInvoicesLookupUrl(params);        
        return buildInvoicesLookupAnchorHtml(url);
    }
    
    protected Properties builInvoicesLookupUrlParameters(BusinessObject bo){
    ...
    protected String buildInvoicesLookupUrl(Properties params){
        String url = UrlFactory.parameterizeUrl(KFSConstants.LOOKUP_ACTION, params);
        String riceHost = configurationService.getPropertyValueAsString(KFSConstants.RICE_SERVER_URL_KEY);
        url = riceHost + "/" + url;
        return url;
    }
    
    protected AnchorHtmlData buildInvoicesLookupAnchorHtml(String url){
        AnchorHtmlData invoicesLookupUrl = new AnchorHtmlData(url, KFSConstants.SEARCH_METHOD, "View Invoices");
        invoicesLookupUrl.setTarget(AnchorHtmlData.TARGET_BLANK);
        return invoicesLookupUrl;
    }
