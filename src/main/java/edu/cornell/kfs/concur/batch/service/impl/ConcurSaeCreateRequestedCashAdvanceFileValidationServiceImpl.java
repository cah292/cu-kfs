...
public class ConcurSaeCreateRequestedCashAdvanceFileValidationServiceImpl implements ConcurSaeCreateRequestedCashAdvanceFileValidationService {
    ...
    private boolean requestDetailFileLineIsCashAdvance(ConcurStandardAccountingExtractDetailLine detailFileLine) {
        if (StringUtils.isNotEmpty(detailFileLine.getCashAdvancePaymentCodeName()) 
            && StringUtils.equalsIgnoreCase(detailFileLine.getCashAdvancePaymentCodeName(), ConcurConstants.SAE_REQUESTED_CASH_ADVANCE_INDICATOR)) {
    ...
    private boolean requestedCashAdvanceApprovedByConcurAdministrator(ConcurStandardAccountingExtractDetailLine detailFileLine) {
        if (StringUtils.isNotEmpty(detailFileLine.getCashAdvanceTransactionType()) 
            && StringUtils.equalsIgnoreCase(detailFileLine.getCashAdvanceTransactionType(), ConcurConstants.SAE_REQUESTED_CASH_ADVANCE_APPROVED_BY_CONCUR_ADMIN)) {
    ...
    private boolean requestedCashAdvanceIsNotBeingDuplicated(ConcurStandardAccountingExtractDetailLine detailFileLine) {
        ...
        if (haveDataForLookup) {
            ...
            cashAdvanceSearchKeys.setRequestId(detailFileLine.getKfsAssembledRequestId());
    ...
    private boolean cashAdvanceKeyIsValid(ConcurStandardAccountingExtractDetailLine detailFileLine) {
        if (StringUtils.isEmpty(detailFileLine.getCashAdvanceKey()) || StringUtils.isEmpty(detailFileLine.getKfsAssembledRequestId())) {
    ...
    private boolean validateAddressIfCheckPayment(ConcurStandardAccountingExtractDetailLine detailFileLine) {
        boolean valid = true;
